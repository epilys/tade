...
from tade.models import Comment, Story
...
#        cursor.execute(
#            f"""CREATE TEMP TRIGGER IF NOT EXISTS on_insert_story_ins AFTER INSERT ON main.tade_storyremotecontent FOR EACH ROW
# BEGIN
...
#        )
#        cursor.execute(
#            f"""CREATE TEMP TRIGGER IF NOT EXISTS on_insert_story_upd AFTER UPDATE of content ON main.tade_storyremotecontent FOR EACH ROW
# BEGIN
#    INSERT OR IGNORE INTO {config.FTS_STORIES_TABLE_NAME}_content (id) VALUES (NEW.story_id);
#    UPDATE {config.FTS_STORIES_TABLE_NAME}_content SET remote_content = NEW.content WHERE id = NEW.story_id;
#    UPDATE {config.FTS_STORIES_TABLE_NAME} SET remote_content = NEW.content WHERE id = NEW.story_id;
...
#        )
#        cursor.execute(
#            f"""CREATE TEMP TRIGGER IF NOT EXISTS on_insert_story_del AFTER DELETE ON main.tade_storyremotecontent FOR EACH ROW
# BEGIN
#    INSERT OR IGNORE INTO {config.FTS_STORIES_TABLE_NAME}_content (id) VALUES (NEW.story_id);
