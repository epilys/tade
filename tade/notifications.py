    ...
    if config.DETECT_USERNAME_MENTIONS_IN_COMMENTS:
        with connections["default"].cursor() as cursor:
            mentioned_users = User.objects.filter(
                id__in=RawSQL(
                    f"SELECT user.id AS id FROM {config.MENTION_TOKENIZER_NAME}, tade_user AS user, tade_comment AS comment WHERE input = comment.text AND comment.id = %s AND token = user.username",
                    (instance.pk,),
                ),
